...
<div class="page-title-box">
    <div class="row align-items-center">
        <div class="col-md-8">
            <h6 class="page-title">Edit Fact</h6>
        </div>
        <div class="col-md-4">
                        ...
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                    <form action="{{url('admin/edit-facts')}}" method="POST" class="outer-repeater col-lg-6 " enctype="mulfactart/form-data">
                                        @csrf
                                        <input type="text" name="id" value="{{$fact_data->fact_id}}" hidden />

                                        <div data-repeater-list="outer-group" class="outer">
                                            <div data-repeater-item class="outer">
                                                <div class="mb-3">
                                                    <label class="form-label" for="formname">Title <spna class="text-danger" >*</span> :</label>
                                                    <input type="text" class="form-control" required   name="fact_title" id="fact_title" onchange="title_canvas()" value="{{$fact_data->fact_title}}" onkeyup="title_canvas()" placeholder="Enter Title...">
                                                    @error('fact_title')
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control" required name="fact_author"  id="fact_author" onchange="fact_author_canvas()" value="{{$fact_data->fact_author}}" onkeyup="fact_author_canvas()" placeholder="Enter author name" />
                                                    @error('fact_author')
                                                    <div class="text text-danger" >
                                                    {{$message}}
                                                    </div>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <textarea rows = '5' cols = '10' name = "fact" required id="fact" type="text" placeholder = "Enter the fact" class = "form-control"
                                                    onkeyup="fact_canvas()">{{$fact_data->fact}}</textarea>
                                                    @error('fact')
                                                    <div class="text text-danger" >
                                                    {{$message}}
                                                    </div>
...
<script>
    ...
    input.focus();
        var fact_data = "{{$fact_data->fact_title}}";
        var fact_title = `{{$fact_data->fact_title}}`;
        var author_name = `{{$fact_data->fact_author}}`;
        var fact_description = `{{$fact_data->fact}}`;
        if(fact_data != ""){
            drawImage(fact_title,author_name,fact_description);
            drawshareImage(fact_title,author_name,fact_description);
        }else{
            drawImage("","","");
            drawshareImage("","","");
        }
